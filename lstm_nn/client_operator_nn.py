...
with open(PATH_TO_CLIENTS_DS, 'r', encoding='utf-8') as f:
...
def write_phrase(phrase, pred):
    ...
    if len(phrase.split(' ')) > 1:
        if (phrase.lower() not in exception_list) and len(phrase):
            if pred[0] > 0.01:
                with open(PATH_TO_NEW_OPERATOR_DS, 'a', encoding='utf-8') as file:
                    file.write(phrase + '\n')
            else:
                with open(PATH_TO_NEW_CLIENTS_DS, 'a', encoding='utf-8') as file:
                    file.write(phrase + '\n')
...
if __name__ == '__main__':
    ...
    # text_list = [
    #     'здравствуйте меня зовут татьяна наталья скажите пожалуйста вам давайте ознакомимся с персональными условия открытия расчётного счета для вашего бизнеса от банка точка',
    #     'Давайте я вам тогда позвоню',
    #     'Точка банк',
    #     'Не звоните сюда больше',
    #     'Я только рассматриваю Тинькоф или Сбер',
    #     'Нет мне сейчас не удобно давайте позже'
    # ]
